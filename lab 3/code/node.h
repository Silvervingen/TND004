...
struct BinarySearchTree<Comparable>::Node {
	...
	Node* parent;  // pointer to parent sub-tree

    // Constructors
    Node(const Comparable& theElement, Node* lt = nullptr, Node* rt = nullptr, Node* ptr = nullptr)
		: element{ theElement }, left{ lt }, right{ rt }, parent{ptr} {
        ++count_nodes;
    }
